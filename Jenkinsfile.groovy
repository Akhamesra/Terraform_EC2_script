    ...
    stages{
        ...
        stage ("terraform apply") {
            steps {
                script{
                    if(run=='Launch'){
                        def flaskOutput = sh(returnStdout: true, script: 'flask ec2 choosesubnet -i '+instancecountnumber).trim()
                        def json = new groovy.json.JsonSlurper().parseText(flaskOutput)
